...
async function compressImage(file) {
  ...
  try { source = await createImageBitmap(file, { imageOrientation:'from-image' }); }
    ...
    source = await new Promise((resolve, reject) => {
      ...
      image.onerror = () => { URL.revokeObjectURL(url); reject(new Error('Cette photo ne peut pas être ouverte. Essaie une autre image.')); };
      image.src = url;
    });
  }
  const width = source.width || source.naturalWidth; const height = source.height || source.naturalHeight;
  const scale = Math.min(1, 1200 / Math.max(width, height)); const canvas = document.createElement('canvas');
